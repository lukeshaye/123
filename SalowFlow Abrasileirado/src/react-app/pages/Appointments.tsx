...
interface AppointmentFormData {
  ...
  service: string;
  price: number;
  professional_id: number; // Alterado de 'professional' para 'professional_id'
...
const defaultFormValues: Partial<AppointmentFormData> = {
    ...
    service: '',
    price: undefined,
    professional_id: undefined, // Alterado
    appointment_date: '',
    end_date: '',
    attended: false,
};
...
export default function Appointments() {
  ...
  const [selectedProfessionalId, setSelectedProfessionalId] = useState<number | 'all'>('all');
  ...
  const onSubmit = async (data: AppointmentFormData) => {
    ...
    try {
      if (editingAppointment) {
        ...
        // Omitimos client_name porque o store irá adicioná-lo
        const { client_name, ...rest } = appointmentData;
        await addAppointment(rest, user.id);
  ...
  const calendarEvents: CalendarEvent[] = useMemo(() => {
    return filteredAppointments.map((appointment: AppointmentType) => {
      ...
      return {
        id: appointment.id!,
        title: `${appointment.client_name} - ${professional?.name || 'N/A'}`,
        start,
        end,
    ...
    <Layout>
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="sm:flex sm:items-center sm:justify-between">
          ...
          <div className="mt-4 sm:mt-0 sm:ml-16 flex items-center space-x-4">
            <div className="relative">
              ...
              <select
                ...
                onChange={(e) => setSelectedProfessionalId(e.target.value === 'all' ? 'all' : Number(e.target.value))}
